...
public:
    ...
    static void entityRewrite210Server(int packetId, DataBuffer *buffer, int currentEntityId, int targetEntitryId){ //TODO 0x12 -> Inv 4 horse | 0x2C 4 combat event | 0x31 Entity effect | 0x3B Attach entity | 0x49 Collect item
    ...
    static void entityRewrite210Client(int packetId, DataBuffer *buffer, int currentEntityId, int targetEntitryId){ //TODO Nothink
        if(packetId == 0x14){
            replaceEntityId(packetId,buffer,currentEntityId,targetEntitryId);
        }
    }

    static void entityRewrite110Server(int packetId, DataBuffer *buffer, int currentEntityId, int targetEntitryId){ //TODO 0x13 -> Inv 4 horse | 0x2C 4 combat event | 0x30 Destroy entity | 0x3B Attach entity | 0x48 Collect item
        if(packetId >= 0x00 && packetId <= 0x06 || packetId == 0x08 || packetId == 0x1B || packetId >= 0x25 && packetId <= 0x28 || packetId == 0x2F || packetId == 0x31 || packetId == 0x34 || packetId == 0x36
    ...
    static void entityRewrite110Client(int packetId, DataBuffer *buffer, int currentEntityId, int targetEntitryId){ //TODO Nothink
        if(packetId == 0x14){
            replaceEntityId(packetId,buffer,currentEntityId,targetEntitryId);
        }
    }
